...
DatabaseVersionBase::DatabaseVersionBase() : _timestamp(mongo::idl::preparsedValue<decltype(_timestamp)>()), _lastMod(mongo::idl::preparsedValue<decltype(_lastMod)>()), _hasTimestamp(false), _hasLastMod(false) {
    // Used for initialization only
}
DatabaseVersionBase::DatabaseVersionBase(mongo::Timestamp timestamp, std::int32_t lastMod) : _timestamp(std::move(timestamp)), _lastMod(std::move(lastMod)), _hasTimestamp(true), _hasLastMod(true) {
    // Used for initialization only
}
...
void DatabaseVersionBase::serialize(BSONObjBuilder* builder) const {
    invariant(_hasTimestamp && _hasLastMod);
