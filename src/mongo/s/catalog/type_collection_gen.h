...
public:
    ...
    static constexpr auto kNoAutoSplitFieldName = "noAutoSplit"_sd;
    ...
    static constexpr auto kSupportingLongNameFieldName = "supportingLongName"_sd;
    ...
    /**
     * Contains the time of when the collection was either created.
     */
     ...
     */
    const boost::optional<std::int64_t> getMaxChunkSizeBytes() const& { return _maxChunkSizeBytes; }
    void getMaxChunkSizeBytes() && = delete;
    void setMaxChunkSizeBytes(boost::optional<std::int64_t> value) & {  _maxChunkSizeBytes = std::move(value);  }

    /**
     * Specifies whether the auto-splitter should be running or not for this collection.
     */
    bool getNoAutoSplit() const { return _noAutoSplit; }
    void setNoAutoSplit(bool value) & {  _noAutoSplit = std::move(value);  }
    ...
    void setTimeseriesFields(boost::optional<mongo::TypeCollectionTimeseriesFields> value) & {  _timeseriesFields = std::move(value);  }
            ...
            /**
             * When set, the maximum name length of sharded collections is aligned with that of non-sharded collections, which is 255 bytes from MongoDB version 4.4 and 120 bytes with earlier versions. Otherwise, the limit for shared collections is reduced by 20 bytes, regardless of the version of MongoDB.
    With version 5.1, the support for long names is implicitly enabled on all collections by upgrading the feature compatibility version (FCV). With FCV lower than 5.1 but using binaries supporting the feature, the support for long names is disabled by default and requires an explicit enabling on the specific collection. Otherwise, the feature cannot be enabled in any way as unsupported by the binaries.
    From a technical perspective, enabling the feature, the system.cache.chunks.<NS> collections are created or renamed using the sharded collection UUID, instead of the namespace, as part of their names, i.e., system.cache.chunks.<UUID>.
             */
    const boost::optional<mongo::SupportingLongNameStatusEnum> getSupportingLongName() const& { return _supportingLongName; }
    void getSupportingLongName() && = delete;
    void setSupportingLongName(boost::optional<mongo::SupportingLongNameStatusEnum> value) & {  _supportingLongName = std::move(value);  }
...
private:
    ...
    bool _unique{false};
    boost::optional<std::int64_t> _maxChunkSizeBytes;
    bool _noAutoSplit{false};
    ...
    boost::optional<mongo::SupportingLongNameStatusEnum> _supportingLongName;
    bool _hasNss : 1;
