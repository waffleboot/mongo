...
constexpr StringData CollectionTypeBase::kMaxChunkSizeBytesFieldName;
constexpr StringData CollectionTypeBase::kNoAutoSplitFieldName;
...
constexpr StringData CollectionTypeBase::kSupportingLongNameFieldName;
...
void CollectionTypeBase::parseProtected(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    ...
    const size_t kPre50CompatibleKeyPatternBit = 5;
    const size_t kPre50CompatibleDefaultCollationBit = 6;
    const size_t kUniqueBit = 7;
    const size_t kMaxChunkSizeBytesBit = 8;
    const size_t kNoAutoSplitBit = 9;
    const size_t kNoBalanceBit = 10;
    const size_t kReshardingFieldsBit = 11;
    const size_t kPre50CompatibleAllowMigrationsBit = 12;
    const size_t kTimeseriesFieldsBit = 13;
    const size_t kSupportingLongNameBit = 14;
    ...
    for (const auto& element :bsonObject) {
        ...
        else if (fieldName == kNoAutoSplitFieldName) {
            if (MONGO_likely(ctxt.checkAndAssertType(element, Bool))) {
                if (MONGO_unlikely(usedFields[kNoAutoSplitBit])) {
                    ctxt.throwDuplicateField(element);
                }

                usedFields.set(kNoAutoSplitBit);

                _noAutoSplit = element.boolean();
            }
        }
        ...
        else if (fieldName == kSupportingLongNameFieldName) {
            if (MONGO_likely(ctxt.checkAndAssertType(element, String))) {
                if (MONGO_unlikely(usedFields[kSupportingLongNameBit])) {
                    ctxt.throwDuplicateField(element);
                }

                usedFields.set(kSupportingLongNameBit);

                IDLParserErrorContext tempContext(kSupportingLongNameFieldName, &ctxt);
                _supportingLongName = SupportingLongNameStatus_parse(tempContext, element.valueStringData());
            }
        }
        else {
...
void CollectionTypeBase::serialize(BSONObjBuilder* builder) const {
    ...
    if (_maxChunkSizeBytes.is_initialized()) {
        builder->append(kMaxChunkSizeBytesFieldName, _maxChunkSizeBytes.get());
    }

    builder->append(kNoAutoSplitFieldName, _noAutoSplit);
    ...
    if (_supportingLongName.is_initialized()) {
        builder->append(kSupportingLongNameFieldName, ::mongo::SupportingLongNameStatus_serializer(_supportingLongName.get()));
    }
