...
constexpr StringData _flushRoutingTableCacheUpdatesWithWriteConcern::kCommandParameterFieldName;
constexpr StringData _flushRoutingTableCacheUpdatesWithWriteConcern::kDbNameFieldName;
constexpr StringData _flushRoutingTableCacheUpdatesWithWriteConcern::kSyncFromConfigFieldName;
constexpr StringData _flushRoutingTableCacheUpdatesWithWriteConcern::kCommandName;

const std::vector<StringData> _flushRoutingTableCacheUpdatesWithWriteConcern::_knownBSONFields {
    _flushRoutingTableCacheUpdatesWithWriteConcern::kSyncFromConfigFieldName,
    _flushRoutingTableCacheUpdatesWithWriteConcern::kCommandName,
};
const std::vector<StringData> _flushRoutingTableCacheUpdatesWithWriteConcern::_knownOP_MSGFields {
    _flushRoutingTableCacheUpdatesWithWriteConcern::kDbNameFieldName,
    _flushRoutingTableCacheUpdatesWithWriteConcern::kSyncFromConfigFieldName,
    _flushRoutingTableCacheUpdatesWithWriteConcern::kCommandName,
};

_flushRoutingTableCacheUpdatesWithWriteConcern::_flushRoutingTableCacheUpdatesWithWriteConcern(const mongo::NamespaceString commandParameter) : _commandParameter(std::move(commandParameter)), _hasDbName(false) {
    // Used for initialization only
}


_flushRoutingTableCacheUpdatesWithWriteConcern _flushRoutingTableCacheUpdatesWithWriteConcern::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto localCmdType = mongo::idl::preparsedValue<mongo::NamespaceString>();
    _flushRoutingTableCacheUpdatesWithWriteConcern object(localCmdType);
    object.parseProtected(ctxt, bsonObject);
    return object;
}
void _flushRoutingTableCacheUpdatesWithWriteConcern::parseProtected(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    std::bitset<2> usedFields;
    const size_t kSyncFromConfigBit = 0;
    const size_t kDbNameBit = 1;
    BSONElement commandElement;
...
_flushRoutingTableCacheUpdatesWithWriteConcern _flushRoutingTableCacheUpdatesWithWriteConcern::parse(const IDLParserErrorContext& ctxt, const OpMsgRequest& request) {
    auto localCmdType = mongo::idl::preparsedValue<mongo::NamespaceString>();
    _flushRoutingTableCacheUpdatesWithWriteConcern object(localCmdType);
    object.parseProtected(ctxt, request);
    return object;
}
void _flushRoutingTableCacheUpdatesWithWriteConcern::parseProtected(const IDLParserErrorContext& ctxt, const OpMsgRequest& request) {
    std::bitset<2> usedFields;
    const size_t kSyncFromConfigBit = 0;
    const size_t kDbNameBit = 1;
    BSONElement commandElement;
...
void _flushRoutingTableCacheUpdatesWithWriteConcern::serialize(const BSONObj& commandPassthroughFields, BSONObjBuilder* builder) const {
    invariant(_hasDbName);
...
OpMsgRequest _flushRoutingTableCacheUpdatesWithWriteConcern::serialize(const BSONObj& commandPassthroughFields) const {
    BSONObjBuilder localBuilder;
    {
        BSONObjBuilder* builder = &localBuilder;
        invariant(_hasDbName);
...
BSONObj _flushRoutingTableCacheUpdatesWithWriteConcern::toBSON(const BSONObj& commandPassthroughFields) const {
    BSONObjBuilder builder;
    serialize(commandPassthroughFields, &builder);
    return builder.obj();
}
