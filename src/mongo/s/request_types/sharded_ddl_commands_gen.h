...
/**
 * Parameters sent by the router
 */
class RefineCollectionShardKeyRequest {
public:
    static constexpr auto kNewShardKeyFieldName = "newShardKey"_sd;

    RefineCollectionShardKeyRequest();
    RefineCollectionShardKeyRequest(mongo::KeyPattern newShardKey);

    static RefineCollectionShardKeyRequest parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject);
    void serialize(BSONObjBuilder* builder) const;
    BSONObj toBSON() const;
...
public:
    ...
    explicit ShardsvrCreateCollection(NamespaceString nss);
...
public:
    ...
    explicit ShardsvrCreateCollectionParticipant(NamespaceString nss);
    ShardsvrCreateCollectionParticipant(NamespaceString nss, std::vector<mongo::BSONObj> indexes, mongo::BSONObj options, mongo::BSONObj idIndex);
...
public:
    ...
    explicit ShardsvrDropCollection(NamespaceString nss);
...
public:
    ...
    explicit ShardsvrDropCollectionParticipant(NamespaceString nss);
...
public:
    ...
    explicit ShardsvrRenameCollection(NamespaceString nss);
...
public:
    ...
    explicit ConfigsvrRenameCollectionMetadata(NamespaceString nss);
    ConfigsvrRenameCollectionMetadata(NamespaceString nss, mongo::NamespaceString to);
...
public:
    ...
    explicit ShardsvrReshardCollection(mongo::NamespaceString commandParameter);
    explicit ShardsvrReshardCollection(mongo::NamespaceString commandParameter, mongo::BSONObj key);
...
public:
    using Reply = void;
    static constexpr auto kRefineCollectionShardKeyRequestFieldName = "RefineCollectionShardKeyRequest"_sd;
    static constexpr auto kDbNameFieldName = "$db"_sd;
    static constexpr auto kNewShardKeyFieldName = "newShardKey"_sd;
    static constexpr auto kCommandName = "_shardsvrRefineCollectionShardKey"_sd;

    explicit ShardsvrRefineCollectionShardKey(NamespaceString nss);
     ...
     * Parameters sent by the router
     */
    const mongo::RefineCollectionShardKeyRequest& getRefineCollectionShardKeyRequest() const { return _refineCollectionShardKeyRequest; }
    mongo::RefineCollectionShardKeyRequest& getRefineCollectionShardKeyRequest() { return _refineCollectionShardKeyRequest; }
    void setRefineCollectionShardKeyRequest(mongo::RefineCollectionShardKeyRequest value) & {  _refineCollectionShardKeyRequest = std::move(value);  }

    /**
     * The index specification document to use as the new shard key.
     */
    const  mongo::KeyPattern& getNewShardKey() const { return _refineCollectionShardKeyRequest.getNewShardKey(); }
    const StringData getDbName() const& { return _dbName; }
    void getDbName() && = delete;
    void setDbName(StringData value) & { auto _tmpValue = value.toString();  _dbName = std::move(_tmpValue); _hasDbName = true; }
...
private:
    ...
    mongo::RefineCollectionShardKeyRequest _refineCollectionShardKeyRequest;
    std::string _dbName;
    bool _hasDbName : 1;
};
...
public:
    ...
    explicit ConfigsvrCreateDatabase(std::string commandParameter);
