...
constexpr StringData RefineCollectionShardKeyRequest::kNewShardKeyFieldName;


RefineCollectionShardKeyRequest::RefineCollectionShardKeyRequest() : _newShardKey(mongo::idl::preparsedValue<decltype(_newShardKey)>()), _hasNewShardKey(false) {
    // Used for initialization only
}
RefineCollectionShardKeyRequest::RefineCollectionShardKeyRequest(mongo::KeyPattern newShardKey) : _newShardKey(std::move(newShardKey)), _hasNewShardKey(true) {
    // Used for initialization only
}


RefineCollectionShardKeyRequest RefineCollectionShardKeyRequest::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<RefineCollectionShardKeyRequest>();
    object.parseProtected(ctxt, bsonObject);
    return object;
}
void RefineCollectionShardKeyRequest::parseProtected(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    std::bitset<1> usedFields;
...
void RefineCollectionShardKeyRequest::serialize(BSONObjBuilder* builder) const {
    invariant(_hasNewShardKey);
...
BSONObj RefineCollectionShardKeyRequest::toBSON() const {
    BSONObjBuilder builder;
    serialize(&builder);
    return builder.obj();
}
...
constexpr StringData ShardsvrRefineCollectionShardKey::kRefineCollectionShardKeyRequestFieldName;
...
const std::vector<StringData> ShardsvrRefineCollectionShardKey::_knownBSONFields {
    ShardsvrRefineCollectionShardKey::kRefineCollectionShardKeyRequestFieldName,
...
ShardsvrRefineCollectionShardKey::ShardsvrRefineCollectionShardKey(const NamespaceString nss) : _nss(std::move(nss)), _dbName(nss.db().toString()), _hasDbName(true) {
    // Used for initialization only
}
