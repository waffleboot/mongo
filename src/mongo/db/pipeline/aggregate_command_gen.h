...
public:
    ...
    explicit AggregateCommandRequest(NamespaceString nss);
    AggregateCommandRequest(NamespaceString nss, std::vector<mongo::BSONObj> pipeline);
...
private:
    void validateMaxTimeMS(std::int64_t value);
    void validateMaxTimeMS(IDLParserErrorContext& ctxt, std::int64_t value);
