...
MergeAuthzCollectionsCommand::MergeAuthzCollectionsCommand() : _tempUsersCollection(mongo::idl::preparsedValue<decltype(_tempUsersCollection)>()), _tempRolesCollection(mongo::idl::preparsedValue<decltype(_tempRolesCollection)>()), _db(mongo::idl::preparsedValue<decltype(_db)>()), _hasTempUsersCollection(false), _hasTempRolesCollection(false), _hasDb(false), _hasDbName(false) {
    // Used for initialization only
}
MergeAuthzCollectionsCommand::MergeAuthzCollectionsCommand(std::string tempUsersCollection, std::string tempRolesCollection, std::string db) : _tempUsersCollection(std::move(tempUsersCollection)), _tempRolesCollection(std::move(tempRolesCollection)), _db(std::move(db)), _hasTempUsersCollection(true), _hasTempRolesCollection(true), _hasDb(true), _hasDbName(true) {
    // Used for initialization only
}
...
void MergeAuthzCollectionsCommand::parseProtected(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    ...
    if (MONGO_unlikely(!usedFields.all())) {
        if (!usedFields[kTempUsersCollectionBit]) {
            ctxt.throwMissingField(kTempUsersCollectionFieldName);
        }
        if (!usedFields[kTempRolesCollectionBit]) {
            ctxt.throwMissingField(kTempRolesCollectionFieldName);
        }
...
void MergeAuthzCollectionsCommand::serialize(const BSONObj& commandPassthroughFields, BSONObjBuilder* builder) const {
    invariant(_hasTempUsersCollection && _hasTempRolesCollection && _hasDb && _hasDbName);
