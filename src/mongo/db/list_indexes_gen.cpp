...
constexpr StringData ListIndexesReplyItem::kOriginalSpecFieldName;
...
void ListIndexesReplyItem::parseProtected(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    ...
    const size_t kOriginalSpecBit = 25;
    const size_t kBuildUUIDBit = 26;
    const size_t kSpecBit = 27;
    ...
    for (const auto& element :bsonObject) {
        ...
        else if (fieldName == kOriginalSpecFieldName) {
            if (MONGO_likely(ctxt.checkAndAssertType(element, Object))) {
                if (MONGO_unlikely(usedFields[kOriginalSpecBit])) {
                    ctxt.throwDuplicateField(element);
                }

                usedFields.set(kOriginalSpecBit);

                const BSONObj localObject = element.Obj();
                _originalSpec = BSONObj::getOwned(localObject);
            }
        }
...
void ListIndexesReplyItem::serialize(BSONObjBuilder* builder) const {
    ...
    if (_originalSpec.is_initialized()) {
        builder->append(kOriginalSpecFieldName, _originalSpec.get());
    }
