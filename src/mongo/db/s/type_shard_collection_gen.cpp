...
constexpr StringData ShardCollectionTypeBase::kAllowAutoSplitFieldName;
...
constexpr StringData ShardCollectionTypeBase::kMaxChunkSizeBytesFieldName;
...
constexpr StringData ShardCollectionTypeBase::kSupportingLongNameFieldName;
...
void ShardCollectionTypeBase::parseProtected(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    ...
    const size_t kMaxChunkSizeBytesBit = 11;
    const size_t kAllowAutoSplitBit = 12;
    const size_t kPre50CompatibleAllowMigrationsBit = 13;
    const size_t kTimeseriesFieldsBit = 14;
    const size_t kSupportingLongNameBit = 15;
    ...
    for (const auto& element :bsonObject) {
        ...
        else if (fieldName == kAllowAutoSplitFieldName) {
            if (MONGO_likely(ctxt.checkAndAssertType(element, Bool))) {
                if (MONGO_unlikely(usedFields[kAllowAutoSplitBit])) {
                    ctxt.throwDuplicateField(element);
                }

                usedFields.set(kAllowAutoSplitBit);

                _allowAutoSplit = element.boolean();
            }
        }
    ...
    if (MONGO_unlikely(!usedFields.all())) {
        ...
        if (!usedFields[kAllowAutoSplitBit]) {
            _allowAutoSplit = true;
        }
    }
...
void ShardCollectionTypeBase::serialize(BSONObjBuilder* builder) const {
    ...
    builder->append(kAllowAutoSplitFieldName, _allowAutoSplit);
