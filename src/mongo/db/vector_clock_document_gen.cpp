...
VectorClockDocument::VectorClockDocument() : _configTime(mongo::idl::preparsedValue<decltype(_configTime)>()), _topologyTime(mongo::idl::preparsedValue<decltype(_topologyTime)>()), _hasConfigTime(false), _hasTopologyTime(false) {
    // Used for initialization only
}
VectorClockDocument::VectorClockDocument(mongo::Timestamp configTime, mongo::Timestamp topologyTime) : _configTime(std::move(configTime)), _topologyTime(std::move(topologyTime)), _hasConfigTime(true), _hasTopologyTime(true) {
    // Used for initialization only
}
...
void VectorClockDocument::parseProtected(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    ...
    if (MONGO_unlikely(!usedFields.all())) {
        ...
        if (!usedFields[kConfigTimeBit]) {
            ctxt.throwMissingField(kConfigTimeFieldName);
        }
        if (!usedFields[kTopologyTimeBit]) {
            ctxt.throwMissingField(kTopologyTimeFieldName);
        }
    }
...
void VectorClockDocument::serialize(BSONObjBuilder* builder) const {
    invariant(_hasConfigTime && _hasTopologyTime);
