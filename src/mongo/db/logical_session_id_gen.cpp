...
constexpr StringData InternalSessionFields::kStmtIdFieldName;
constexpr StringData InternalSessionFields::kTxnNumberFieldName;
constexpr StringData InternalSessionFields::kTxnUUIDFieldName;
...
InternalSessionFields InternalSessionFields::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<InternalSessionFields>();
    object.parseProtected(ctxt, bsonObject);
    return object;
}
void InternalSessionFields::parseProtected(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    std::bitset<3> usedFields;
    const size_t kTxnNumberBit = 0;
    const size_t kStmtIdBit = 1;
    const size_t kTxnUUIDBit = 2;

    for (const auto& element :bsonObject) {
        const auto fieldName = element.fieldNameStringData();


        if (fieldName == kTxnNumberFieldName) {
        ...
        else if (fieldName == kTxnUUIDFieldName) {
            if (MONGO_likely(ctxt.checkAndAssertBinDataType(element, newUUID))) {
                if (MONGO_unlikely(usedFields[kTxnUUIDBit])) {
                    ctxt.throwDuplicateField(element);
                }

                usedFields.set(kTxnUUIDBit);

                _txnUUID = UUID(uassertStatusOK(UUID::parse(element)));
            }
        }
        else {
...
void InternalSessionFields::serialize(BSONObjBuilder* builder) const {
    ...
    if (_txnUUID.is_initialized()) {
        ConstDataRange tempCDR = _txnUUID.get().toCDR();
        builder->append(kTxnUUIDFieldName, BSONBinData(tempCDR.data(), tempCDR.length(), newUUID));
    }

}


BSONObj InternalSessionFields::toBSON() const {
    BSONObjBuilder builder;
    serialize(&builder);
    return builder.obj();
}

constexpr StringData LogicalSessionId::kInternalSessionFieldsFieldName;
constexpr StringData LogicalSessionId::kIdFieldName;
constexpr StringData LogicalSessionId::kStmtIdFieldName;
constexpr StringData LogicalSessionId::kTxnNumberFieldName;
constexpr StringData LogicalSessionId::kTxnUUIDFieldName;
    ...
    for (const auto& element :bsonObject) {
        ...
        if (fieldName == kTxnNumberFieldName) {
            if (MONGO_likely(ctxt.checkAndAssertType(element, NumberLong))) {
                ...
                _internalSessionFields.setTxnNumber(boost::optional<std::int64_t>(element._numberLong()));
            }
        }
        else if (fieldName == kStmtIdFieldName) {
            if (MONGO_likely(ctxt.checkAndAssertType(element, NumberInt))) {
                ...
                _internalSessionFields.setStmtId(boost::optional<std::int32_t>(element._numberInt()));
            }
        }
        else if (fieldName == kTxnUUIDFieldName) {
            if (MONGO_likely(ctxt.checkAndAssertBinDataType(element, newUUID))) {
                ...
                _internalSessionFields.setTxnUUID(boost::optional<mongo::UUID>(UUID(uassertStatusOK(UUID::parse(element)))));
            }
        }
...
void LogicalSessionId::serialize(BSONObjBuilder* builder) const {
    ...
    {
        _internalSessionFields.serialize(builder);
    }
...
constexpr StringData LogicalSessionFromClient::kInternalSessionFieldsFieldName;
constexpr StringData LogicalSessionFromClient::kIdFieldName;
constexpr StringData LogicalSessionFromClient::kStmtIdFieldName;
constexpr StringData LogicalSessionFromClient::kTxnNumberFieldName;
constexpr StringData LogicalSessionFromClient::kTxnUUIDFieldName;
