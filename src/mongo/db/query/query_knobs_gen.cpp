...
AtomicWord<long long> internalMaxSubPipelineViewDepth{20};
...
AtomicDouble internalQuerySBEAggMemoryUseSampleRate{0.5};
AtomicWord<long long> internalQuerySBEAggApproxMemoryUseInBytesBeforeSpill{1024 * 1024};
...
AtomicWord<int> internalQueryMaxNAccumulatorBytes{100 * 1024 * 1024};
...
MONGO_SERVER_PARAMETER_REGISTER(idl_aa629c1d5e7ce5614728bd1d4f3e04442ceffe4e)(InitializerContext*) {
    ...
    /**
     * The maximum length allowed for an an aggregation sub-pipeline view.
     */
    auto* scp_12 = ([]() -> ServerParameter* {
        auto* ret = makeIDLServerParameterWithStorage<ServerParameterType::kStartupAndRuntime>("internalMaxSubPipelineViewDepth", internalMaxSubPipelineViewDepth);
        ret->addBound<idl_server_parameter_detail::GT>(0);
        return ret;
    })();
    scp_12->setTestOnly();
    ...
    /**
     * The percent chance that the size of the hash table in a HashAgg stage is re-estimated when an entry is updated. The estimated size of the hash table is used to determine if we should spill to disk. [see internalQuerySlotBasedExecutionHashAggApproxMemoryUseInBytesBeforeSpill]
     */
    [[maybe_unused]] auto* scp_45 = ([]() -> ServerParameter* {
        auto* ret = makeIDLServerParameterWithStorage<ServerParameterType::kStartupAndRuntime>("internalQuerySlotBasedExecutionHashAggMemoryUseSampleRate", internalQuerySBEAggMemoryUseSampleRate);
    ...
    /**
     * The max size in bytes that the hash table in a HashAgg stage can be estimated to be before we spill to disk.
     */
    [[maybe_unused]] auto* scp_46 = ([]() -> ServerParameter* {
        auto* ret = makeIDLServerParameterWithStorage<ServerParameterType::kStartupAndRuntime>("internalQuerySlotBasedExecutionHashAggApproxMemoryUseInBytesBeforeSpill", internalQuerySBEAggApproxMemoryUseInBytesBeforeSpill);
        ret->addBound<idl_server_parameter_detail::GT>(0);
        return ret;
    })();
    ...
    /**
     * Limits the vector of values pushed into a single array while grouping with the 'N' family of accumulators.
     */
    [[maybe_unused]] auto* scp_49 = ([]() -> ServerParameter* {
        auto* ret = makeIDLServerParameterWithStorage<ServerParameterType::kStartupAndRuntime>("internalQueryMaxNAccumulatorBytes", internalQueryMaxNAccumulatorBytes);
        ret->addBound<idl_server_parameter_detail::GT>(0);
        return ret;
    })();
     ...
     */
    [[maybe_unused]] auto* scp_50 = ([]() -> ServerParameter* {
