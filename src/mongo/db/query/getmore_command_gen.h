...
public:
    ...
    explicit GetMoreCommandRequest(std::int64_t commandParameter);
    explicit GetMoreCommandRequest(std::int64_t commandParameter, std::string collection);
    ...
    /**
     * The batch size is optional. If not provided, we will put as many documents into the batch as fit within the byte limit.
    ...
    /**
     * The await data timeout: the number of milliseconds for which a getMore on a tailable, awaitData query should block.
